...
class State(BaseModel):
    ...
    def to_cdf(self, client: CogniteClient) -> None:
        client.raw.rows.insert(database=RAW_DATABASE, table=RAW_TABLE, row_key=self.key, columns=self.model_dump())
...
) -> Iterable[list[CogniteDiagramAnnotationApply]]:
...
def to_direct_relations_by_source_by_node(
    annotations: list[CogniteDiagramAnnotationApply], mappings: list[Mapping], logger: CogniteFunctionLogger
    ...
    for annotation in annotations:
        ...
        annotation_by_source_by_node[entity_source][(node, direct_relation_property)].append(
    ...
    return dm.query.Query(
        ...
        select={
            "annotations": dm.query.Select(
                [dm.query.SourceSelector(source=CogniteDiagramAnnotationApply.get_source(), properties=["*"])]
            )
        },
