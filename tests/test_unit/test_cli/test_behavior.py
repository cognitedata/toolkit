...
def test_inject_custom_environmental_variables(
    ...
    config_yaml = yaml.safe_load((builtin_legacy_org_dir / "config.dev.yaml").read_text())
...
def test_pull_dataset(
    ...
    toolkit_client_approval: ApprovalToolkitClient,
    env_vars_with_client: EnvironmentVariables,
    builtin_legacy_org_dir_mutable: Path,
) -> None:
    # Loading a selected dataset to be pulled
    dataset_yaml = builtin_legacy_org_dir_mutable / MODULES / "cdf_common" / "data_sets" / "demo.DataSet.yaml"
    ...
    cmd.pull_module(
        ...
        organization_dir=builtin_legacy_org_dir_mutable,
        env="dev",
        dry_run=False,
        verbose=False,
        env_vars=env_vars_with_client,
...
def test_pull_dataset_relative_path(
    ...
    with chdir(builtin_legacy_org_dir_mutable):
        cmd = PullCommand(silent=True)
        cmd.pull_module(
...
def test_pull_transformation_sql(
    ...
    transformation_yaml = (
        builtin_legacy_org_dir_mutable
        / "modules"
        / "cdf_pi"
        / "transformations"
        / "population"
...
def test_pull_workflow_trigger_with_environment_variables(
    ...
    yaml_filepath = (
        builtin_legacy_org_dir_mutable / "modules" / "cdf_ingestion" / "workflows" / "trigger.WorkflowTrigger.yaml"
    )
