...
def valid_robot_capability_cases() -> Iterable:
    yield pytest.param(
        {
            "name": "PTZ Camera",
    ...
    yield pytest.param(
        {"name": "Docking Station", "externalId": "dock_001", "method": "return_to_dock"}, id="minimal-robot-capability"
    )


def invalid_robot_capability_cases() -> Iterable:
    yield pytest.param(
...
class TestRobotCapabilityYAML:
    ...
    @pytest.mark.parametrize("data", valid_robot_capability_cases())
    def test_load_valid_robot_capability(self, data: dict[str, object]) -> None:
        loaded = RobotCapabilityYAML.model_validate(data)
        assert loaded.model_dump(exclude_unset=True, by_alias=True) == data

    @pytest.mark.parametrize("data, expected_errors", invalid_robot_capability_cases())
