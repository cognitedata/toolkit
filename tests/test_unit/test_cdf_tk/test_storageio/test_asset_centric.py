...
class TestFileMetadataIO:
    ...
    def test_download_upload_command(self, some_filemetadata_data: FileMetadataList, tmp_path: Path) -> None:
        selector = DataSetSelector(data_set_external_id="test_data_set")
        with monkeypatch_toolkit_client() as client:
            client.files.return_value = [some_filemetadata_data]
            client.files.aggregate_count.return_value = 50
            client.lookup.data_sets.external_id.return_value = "test_data_set"
            client.lookup.data_sets.id.return_value = 1234
            client.lookup.assets.external_id.return_value = "test_hierarchy"
            client.lookup.assets.id.return_value = 123
            ...
            args, _ = client.files.create.call_args
            assert len(args) == 1
            uploaded_files = args[0]
            from cognite.client.data_classes import FileMetadataWriteList

            assert isinstance(uploaded_files, FileMetadataWriteList)
            assert [file.dump() for file in uploaded_files] == [
                file.as_write().dump() for file in some_filemetadata_data
            ]
