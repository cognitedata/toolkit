...
class TestChartIO:
    def test_download_chart(self, toolkit_config: ToolkitClientConfig) -> None:
        ...
        with responses.RequestsMock() as rsps:
            ...
            chart_data = [
                io.json_chunk_to_data(
                    [(f"item {item_no} in chunk {chunk_no}", item) for item_no, item in enumerate(chunk)]
                )
                for chunk_no, chunk in enumerate(json_iterator)
            ]

            assert len(chart_data) == 1
            chart_list = chart_data[0]
            assert len(chart_list) == 2
            first = chart_list[0].item
