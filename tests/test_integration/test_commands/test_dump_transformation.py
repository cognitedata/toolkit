...
class TestDumpTransformation:
    def test_dump_transformation_with_schedule_and_notification(
        ...
        assert sum(1 for _ in transformation_folder.glob(f"*{TransformationIO.kind}.yaml")) == 1
        assert sum(1 for _ in transformation_folder.glob(f"*{TransformationScheduleIO.kind}.yaml")) == 1
        assert sum(1 for _ in transformation_folder.glob(f"*{TransformationNotificationIO.kind}.yaml")) == 1
