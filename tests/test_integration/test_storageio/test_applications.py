...
@pytest.fixture
def canvas_event(toolkit_client: ToolkitClient, toolkit_dataset: DataSet) -> EventResponse:
    event = EventRequest(
    ...
    if existing := toolkit_client.tool.events.retrieve([event.as_id()]):
        return existing[0]
    created_event = toolkit_client.tool.events.create([event])
    assert len(created_event) == 1, f"Expected to create 1 event, but got {len(created_event)}"
    return created_event[0]


@pytest.fixture
def canvas_raw_data(canvas_event: EventResponse) -> dict[str, Any]:
...
class TestCanvasIO:
    def test_upload_canvas(self, toolkit_client: ToolkitClient, canvas_raw_data: dict[str, Any]) -> None:
        ...
        not_success = [message for message in upload_result if not isinstance(message, ItemsSuccessResponse)]
