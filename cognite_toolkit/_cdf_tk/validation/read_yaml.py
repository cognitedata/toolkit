...
def _read_parameter_from_init_type_hints(cls_: type, path: tuple[str | int, ...], seen: set[str]) -> ParameterSpecSet:
    ...
    for name, parameter in parameters.items():
        ...
        if hint.is_base_type or str(hint) in seen:
            ...
            key, value = hint.args
            dict_set = _read_parameter_from_init_type_hints(value, (*path, name), seen)
        ...
        if hint.is_list_type:
            ...
            elif hint.args[0].__name__ in seen:
                parameter_set.add(ParameterSpec((*path, name, 0), "dict", is_required, is_nullable))
            else:
                list_set = _read_parameter_from_init_type_hints(hint.args[0], (*path, name, 0), seen)
                parameter_set.update(list_set)
        elif hint.is_class:
            cls_set = _read_parameter_from_init_type_hints(hint.arg, (*path, name), seen)
