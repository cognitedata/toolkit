...
class AssetCentricMigrationIOAdapter(
    ...
    @staticmethod
    def _get_asset_centric_resource_id(item: InstanceApply) -> int:
    ...
    @staticmethod
    def _get_asset_centric_resource_id_dumped_instance(item: dict[str, JsonVal] | object) -> int | None:
        if not isinstance(item, dict):
            return None
        sources = item.get("sources")
        if not isinstance(sources, list):
            return None
        instance_source_dump = INSTANCE_SOURCE_VIEW_ID.dump()
        source = next(
            (source for source in sources if isinstance(source, dict) and source.get("source") == instance_source_dump),
            None,
        )
        if source is None:
            return None
    ...
    def as_id(self, item: dict[str, JsonVal] | object) -> int:
        if isinstance(item, AssetCentricMapping):
            ...
            return self._get_asset_centric_resource_id(item)
        elif isinstance(item, dict) and isinstance(item.get("id"), int):
            # MyPy checked above.
            return item["id"]  # type: ignore[arg-type, return-value]
        elif id_ := self._get_asset_centric_resource_id_dumped_instance(item):
            return id_
