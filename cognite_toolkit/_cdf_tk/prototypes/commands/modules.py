...
class ModulesCommand(ToolkitCommand):
    ...
    def _create(
        ...
        _system_yaml_file = Path.cwd() / "cognite_toolkit" / "_system.yaml"
        _system_yaml = yaml.safe_load(_system_yaml_file.read_text())
        _system_yaml.pop("packages", None)

        Path(init_dir + "/system.yaml").write_text(f"#DO NOT EDIT THIS FILE!\n{yaml.dump(_system_yaml)}")
