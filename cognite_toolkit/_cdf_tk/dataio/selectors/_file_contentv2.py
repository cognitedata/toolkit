...
class NodeIdWithName(NodeId):
    ...
    @property
    def display_name(self) -> str:
        return f"{self.name} ({self.space}:{self.external_id})"
...
class CogniteFileContentSelectorV2(DataSelector, ABC):
    kind: Literal["CogniteFileContent"] = "CogniteFileContent"


class CogniteFileTemplateV2(SelectorObject):
    model_config = ConfigDict(extra="allow")
    space: str
    external_id: str
    ...
    @classmethod
    def _validate_filename_in_external_id(cls, v: str) -> str:
        if FILENAME_VARIABLE not in v:
            raise ValueError(
                f"{FILENAME_VARIABLE!s} must be present in 'external_id'. "
                f"This allows for dynamic substitution based on the file name."
            )
        return v


class CogniteFileTemplateSelectorV2(CogniteFileContentSelectorV2):
    type: Literal["CogniteFileTemplate"] = "CogniteFileTemplate"
    template: CogniteFileTemplateV2
    file_directory: DirectoryPath
    guess_mime_type: bool
...
class CogniteFileFilesSelectorV2(CogniteFileContentSelectorV2):
    """Download/upload CogniteFile nodes with file content.

    For download, the ids field must be set (node space, external id, and display name).
    For upload, all rows in a csv/parquet file are uploaded.
