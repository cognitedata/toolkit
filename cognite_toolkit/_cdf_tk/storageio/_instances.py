...
class InstanceIO(
    ...
    def stream_data(self, selector: InstanceSelector, limit: int | None = None) -> Iterable[Page[Instance]]:
        if isinstance(selector, InstanceViewSelector | InstanceSpaceSelector):
            ...
            for instance in iterate_instances(client=self.client, **selector.as_filter_args()):
                ...
                if len(chunk) >= self.CHUNK_SIZE:
                    yield Page(worker_id="-1", items=chunk)
                    chunk = InstanceList([])
            if chunk:
                yield Page(worker_id="-1", items=chunk)
    ...
    def json_to_resource(self, item_json: dict[str, JsonVal]) -> InstanceApply:
        ...
        if self._remove_existing_version and "existingVersion" in item_json:
            del item_json["existingVersion"]
        if instance_type == "node":
            return NodeApply._load(item_json, cognite_client=self.client)
        elif instance_type == "edge":
            return EdgeApply._load(item_json, cognite_client=self.client)
        else:
