...
class ChartIO(UploadableStorageIO[ChartSelector, Chart, ChartWrite]):
    ...
    def stream_data(self, selector: ChartSelector, limit: int | None = None) -> Iterable[Page[Chart]]:
        ...
        for chunk in chunker_sequence(selected_charts, self.CHUNK_SIZE):
            ...
            yield Page(worker_id="-1", items=ChartList(chunk))
...
class CanvasIO(UploadableStorageIO[CanvasSelector, IndustrialCanvas, IndustrialCanvasApply]):
    ...
    def stream_data(self, selector: CanvasSelector, limit: int | None = None) -> Iterable[Page[IndustrialCanvas]]:
