...
class MigrateApp(typer.Typer):
    ...
    def _resolve_infield_migration_spaces(
        *,
        ...
        verbose: bool,
        label: str,
    ) -> tuple[str, str, Path, bool, bool]:
        """Select/validate source and target spaces for Infield data migrations.

        If neither ``--source-space`` nor ``--target-space`` is provided, both are selected interactively.
        If both are provided, they are validated against the candidate spaces. Providing only one is an error.
        """
