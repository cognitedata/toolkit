...
class MigrateApp(typer.Typer):
    ...
    def infield_data(
        ...
        if source_space in shared_legacy_spaces:
            ...
            write_location_split_reports(log_dir, resolution, console=client.console)
            ...
            if target_space is None:
                raise typer.BadParameter("Bug in Toolkit: target space is required for non-split Infield data migration.")
    ...
    def infield_source_data(
        ...
        for space in target_spaces:
            ...
            for type_key, view_id in space_custom_views.items():
                ...
                if existing is not None and existing != view_id:
                    raise ToolkitMigrationError(
                        f"Location split targets disagree on the custom {type_key} view: "
                        f"{existing!s} vs {view_id!s}."
                    )
        ...
        for mapping in mappings:
            ...
            if source_entity is not None and source_entity in source_views:
                updated_source_mappings.append(
                    mapping.model_copy(update={"source_view": source_views[source_entity]})
                )
            else:
...
def _resolve_infield_migration_spaces(
    *,
    ...
    if source_space is None and target_space is None:
        ...
        log_dir = Path(questionary.path("Specify log directory for migration logs:", default=str(log_dir)).unsafe_ask())
        dry_run = questionary.confirm("Do you want to perform a dry run?", default=dry_run).unsafe_ask()
        verbose = questionary.confirm("Do you want verbose output?", default=verbose).unsafe_ask()
