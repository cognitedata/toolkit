...
AuthLoginFlowCli: TypeAlias = Literal["session", "device-code", "client-credentials", "token", "interactive"]
VALID_PROVIDERS = get_args(Provider)
VALID_LOGIN_FLOWS = get_args(LoginFlow)
VALID_AUTH_LOGIN_FLOWS = get_args(AuthLoginFlowCli)

CLIENT_NAME = f"CDF-Toolkit:{__version__}"

_AUTH_LOGIN_FLOW_TO_LOGIN_FLOW: dict[AuthLoginFlowCli, LoginFlow] = {
    "session": "session",
...
def parse_auth_login_flow(flow: AuthLoginFlowCli) -> LoginFlow:
    return _AUTH_LOGIN_FLOW_TO_LOGIN_FLOW[flow]


def read_env_login_flow() -> LoginFlow | None:
    login_flow = os.environ.get("LOGIN_FLOW", "").strip().lower()
    if login_flow in VALID_LOGIN_FLOWS:
        return login_flow  # type: ignore[return-value]
    return None
