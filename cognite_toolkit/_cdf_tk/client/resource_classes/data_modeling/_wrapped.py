"""This module contains base classes for resources that are wrapped around the instance API.

These are for example used for InField configurations that are nodes, but as we support
directly in Toolkit.
"""
...
from cognite_toolkit._cdf_tk.client.identifiers import EdgeReference, NodeReference, ViewReference

from ._constants import InstanceType
...
class WrappedInstanceResponse(ResponseResource[T_WrappedInstanceRequest], ABC):
    ...
    def move_properties(cls, values: dict[str, Any]) -> dict[str, Any]:
        ...
        return _move_properties(values, cls.VIEW_ID)
...
def _move_properties(values: dict[str, Any], view_id: ViewReference) -> dict[str, Any]:
...
class WrappedInstanceListRequest(RequestResource, ABC):
    ...
    @abstractmethod
    def as_ids(self) -> list[NodeReference | EdgeReference]:
