...
COL_KEY_MAN_MAPPING_ASSET_EXTID  = "assetExternalId"  # ExternalID Col name for Assset related to manual mapping
...
def handle(data: dict, client: CogniteClient, secrets: dict, function_call_info: dict) -> dict:
    # Function handler for the contextualization of Time Series and Assets.
    try:
...
def contextualize_ts_and_asset(cognite_client: CogniteClient, config: ContextConfig) -> None:
    ...
    for asset_root_ext_id in config.asset_root_ext_ids:
        try:
            ...
            logger.info(f"Read time series for contextualization data set: {config.time_series_data_set_ext_id}, asset root: {asset_root_ext_id} - read and process all = {config.run_all}")
            ...
            msg = f"Contextualization of TS to asset root: {asset_root_ext_id}, num TS contextualized: {len_good_matches}, num TS NOT contextualized : {len_bad_matches} (score below {config.match_threshold})"
            logger.info(msg)
...
def read_manual_mappings(
    ...
    try:
        ...
        if man_table_true:
            ...
            for row in row_list:
                if config.run_all or (COL_KEY_MAN_CONTEXTUALIZED not in row.columns or row.columns[COL_KEY_MAN_CONTEXTUALIZED] != True):
                    strip_value_rows = {}
...
def apply_manual_mappings(cognite_client: CogniteClient, config: ContextConfig, raw_uploader: RawUploadQueue, manual_mappings: list[Row]) -> None:
    ...
    try:
        ...
        mapping_dict = {mapping[COL_KEY_MAN_MAPPING_TS_EXTID]: mapping[COL_KEY_MAN_MAPPING_ASSET_EXTID] for mapping in manual_mappings}
        ...
        for time_series in ts_list_manual_mapping:
            ...
            time_series.metadata[TS_CONTEXTUALIZED_METADATA_KEY] = f"Manual matched from raw: {config.rawdb} / {config.raw_table_manual}"
...
def get_ts_list_manual_mapping(
    cognite_client: CogniteClient, 
    manual_mappings: list[Row]
) -> list[TimeSeries]:
    ...
    try:      
        ...
        if len(ts_list_manual_mapping) != len(ts_ext_id_list):
            raise ValueError(f"Number of time series found {len(ts_list_manual_mapping)} does not match number of time series in input list {len(ts_ext_id_list)}")
...
def get_time_series(
    ...
    try: 
        ts_list = cognite_client.time_series.list(
            ...
            external_id_prefix=config.time_sereis_prefix
        )
        
        #get list of time series in manual mappings to exclude from matching
        ts_ext_id_manual_match_list = [mmatch[COL_KEY_MAN_MAPPING_TS_EXTID] for mmatch in manual_matches]
        ...
        logger.error(f"Not able to get entities for time sereies in data set: {config.time_series_data_set_ext_id} with prefix: {config.time_sereis_prefix}- error: {e}")
