...
def annotate_3d_model(client: CogniteClient, config: ContextConfig) -> None:
    ...
    try:
        ...
        if len(manual_mappings) >0:
            ...
            existing_matches = apply_manual_mappings(client, config, raw_uploader, manual_mappings, existing_matches, model_id, revision_id, tree_d_nodes)
        ...
        if len(three_d_entities) > 0:
            ...
            good_matches, bad_matches, existing_matches = select_and_apply_matches(client, config, match_results, tree_d_nodes, model_id, revision_id, existing_matches)
            ...
            if config.run_all or not config.keep_old_mapping and len(mapping_to_delete) > 0:
                old_mappings_removed = remove_old_mappings(client, mapping_to_delete, existing_matches, model_id, revision_id)
