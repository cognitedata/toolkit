...
from cognite.client.data_classes.three_d import ThreeDModel, ThreeDModelRevision, ThreeDAssetMapping
...
def get_3d_model_id_and_revision_id(
    ...
    try:
        ...
        model_id_list = [model.id for model in client.three_d.models.list(published=True, limit=1) if model.name == three_d_model_name]
        ...
        raise Exception(f"ERROR: Not able to get entities for 3D nodes in data set: {config.three_d_data_set_ext_id}- error: {e}")
    
def  get_mapping_to_delete(
    client: CogniteClient,
    model_id: int, 
    revision_id: int
) -> list[ThreeDAssetMapping]:
...
def get_treed_asset_mappings(
    ...
    mappings = client.three_d.asset_mappings.list(
            model_id=model_id, revision_id=revision_id, limit=-1
    )
...
def filter_3d_nodes(
    ...
    try:
        ...
        print(f"INFO: Total number of 3D Node names found for manual mapping: {num_nodes} - unique names : {len(tree_d_nodes)}")
...
def get_3d_nodes(
    ...
    try:
        ...
        if not input_three_d_nodes:
            ...
            client.files.upload_bytes(file_content, external_id=model_file_name, name=model_file_name, overwrite=True, data_set_id=three_d_data_set_id)
        ...
        print(f"INFO: Total number of 3D Node found: {num_nodes} - unique names to match after asset name filtering: {len(three_d_nodes)}")
...
def get_3d_entities(node: list[str], modNodeName: str, entities: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    entities.append({"id": node['id'], "name": modNodeName, "external_id": node['treeIndex'], "org_name": node['name'], "type": "3dNode"})
    return entities
...
def get_assets(
    ...
    try:
        ...
        raise Exception(f"ERROR: Not able to get entities for asset extId root: {asset_root_ext_id}. Error: {type(e)}({e})")
