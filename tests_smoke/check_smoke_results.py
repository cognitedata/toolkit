...
def check_smoke_tests_results(pytest_report: Path, context: Context) -> None:
    ...
    if not messages:
        if alive_message := _alive_message(context):
        ...
        _notify_slack(
            [
                SlackMessage(
                    ...
                    message=f"[{context.cluster}] Smoke tests failed to execute. Report file {pytest_report.name!r} does not exist. Go to {context.github_repo_url} to investigate.",
                )
            ],
            context,
        )
    ...
    try:
        ...
        _notify_slack(
            [
                SlackMessage(
                    topic="Smoke Test Execution",
                    message=f"[{context.cluster}] Smoke tests failed to execute. Report file {pytest_report.name!r} is not a valid pytest report. Go to {context.github_repo_url} to investigate.",
                )
            ],
            context,
        )
    ...
    for test in report.tests or []:
        ...
        if test.call is None or test.call.crash is None:
            messages.append(
                SlackMessage(
                    topic="Smoke Test Execution",
                    message=f"[{context.cluster}] Test {test.nodeid!r} failed without a crash message. Please investigate and fix the issue. Go to {context.github_repo_url} to investigate.",
                )
            )
            continue
        messages.append(
            SlackMessage(
                topic="CDF API",
                message=f"[{context.cluster}] {test.call.crash.message.removeprefix('AssertionError: ').strip()}",
            )
        )
    return messages


def _alive_message(context: Context) -> SlackMessage | None:
    """Create an "alive" message if the current time is Monday in the morning UTC."""
    if context.now.weekday() == 0 and 2 <= context.now.hour < 10:
        return SlackMessage(
            topic="Smoke Test Execution",
            message=f"[{context.cluster}] Smoke tests are running fine. No issues detected. Have a great week ahead!",
        )
    return None
...
if __name__ == "__main__":
    ...
    if GITHUB_REPO_URL_NAME not in os.environ:
        ...
        sys.exit(1)
    if CDF_CLUSTER_NAME not in os.environ:
        print(f"Environment variable {CDF_CLUSTER_NAME} is not set.")
        sys.exit(1)
    ...
    check_smoke_tests_results(
        ...
        Context(
            ...
            cluster=os.environ[CDF_CLUSTER_NAME],
