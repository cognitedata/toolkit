...
@pytest.fixture(scope="session")
def smoke_view(
    toolkit_client: ToolkitClient, smoke_space: SpaceResponse, smoke_container: ContainerResponse
) -> ViewResponse:
            ...
            ViewRequest(
                ...
                properties={
                    "name": ViewCorePropertyRequest(
                        container=smoke_container.as_id(), container_property_identifier="name"
                    )
                },
            )
        ]
