...
def trigger_diagram_detection_jobs(
    ...
    for file_view, entity_sources in entity_sources_by_file_view.items():
        ...
        for file_node in client.data_modeling.instances.list(
            instance_type="node", space=instance_spaces, filter=is_view, limit=-1
        ):
...
def trigger_detection_job(
    ...
    node_entities = dm.NodeList([node for nodes in query_result.values() for node in nodes])
...
def create_entity_query(entity_sources: list[ViewProperty], cursors: dict[str, str]) -> dm.query.Query:
    return dm.query.Query(
        ...
        cursors={entity.external_id: cursors.get(entity.external_id) for entity in entity_sources},
    )
...
def write_annotations(
    ...
    logger.info(
        f"Created {create_count} updated {update_count}, and {unchanged_count} unchanged annotations for {job.file_id}"
    )
