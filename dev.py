...
def create_changelog_entry() -> None:
    ...
    if not changelog_items[1:]:
        ...
        raise SystemExit(1)
    if not _is_header(changelog_items[1], level=2, text="Changelog"):
        print("The first header in the changelog must be '## Changelog'.")
        raise SystemExit(1)
    change_entries = list(itertools.takewhile(lambda x: not _is_header(x, level=2), changelog_items[2:]))
    _validate_entries(change_entries, "Changelog")

    changelog_entry = _text_after_heading(changelog_text, "Changelog")
    if changelog_entry is None:
        print("The first header in the changelog must be '## Changelog'.")
        raise SystemExit(1)
    CHANGELOG_ENTRY_FILE.write_text(f"## Changelog\n\n{changelog_entry}", encoding="utf-8")
    print(f"Changelog entry written to {CHANGELOG_ENTRY_FILE}.")


def _validate_no_test_plan(pr_description: str) -> None:
    if "This PR was generated by [Mend Renovate]" in pr_description:
        return
    for item in marko.parse(pr_description).children:
        if not isinstance(item, marko.block.Heading) or item.level != 2:
            continue
        if not isinstance(item.children[0], marko.inline.RawText):
            continue
        if item.children[0].children.strip().casefold() == "test plan":
            print("PR description must not contain a '## Test plan' section.")
            raise SystemExit(1)


def _preview_message(text: str, max_lines: int = 20) -> str:
...
def _find_level2_heading(text: str, name: str) -> re.Match | None:
...
def _text_after_heading(text: str, name: str) -> str | None:
    match = _find_level2_heading(text, name)
    if match is None:
        return None
...
def _read_last_commit_message() -> tuple[list[marko.element.Element], str]:
    ...
    _validate_no_test_plan(last_git_message)
    if "This PR was generated by [Mend Renovate]" in last_git_message:
        print("Skipping Renovate PR.")
        changelog_text = "- [ ] Patch\n- [ ] Minor\n- [x] Skip\n"
    else:
        bump_section = _text_after_heading(last_git_message, "Bump")
        if bump_section is None:
            print("No bump entry found in the last commit message.")
            print("Expected a '## Bump' section. Input started with:")
            print(_preview_message(last_git_message))
            raise SystemExit(1)
        changelog_text = bump_section
...
def _is_header(item: marko.element.Element, level: int, text: str | None = None) -> bool:
    if not isinstance(item, marko.block.Heading) or item.level != level:
        return False
    if not item.children or not isinstance(item.children[0], marko.inline.RawText):
        return False
    if text is None:
        return True
    return item.children[0].children.strip().casefold() == text.casefold()
    ...
    for item in items:
        if isinstance(item, marko.block.Heading):
            if last_header:
                ...
                raise SystemExit(1)
            header_text = item.children[0].children.strip()
